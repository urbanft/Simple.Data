...
where object_id = object_id(@tableName, 'TABLE') or object_id = object_id(@tableName, 'VIEW') order by column_id";
        ...
        {
            var primaryKeySql = UFT.Simple.Data.SqlServer.Resources.Resource.PrimaryKeySql;
            return SelectToDataTable(primaryKeySql);
        }

        private DataTable GetForeignKeys()
        {
            return SelectToDataTable(UFT.Simple.Data.SqlServer.Resources.Resource.ForeignKeysSql);
        }
